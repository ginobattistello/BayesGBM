...
BayesGBM is a research toolbox for fitting generative brain/behavior models,
...
BayesGBM is scientifically derived from the Computational Behavioral Modeling
...
The observation function returns:

- Gaussian: predicted mean;
- Bernoulli: `P(y=1)`;
- categorical: a probability vector.

BayesGBM constructs the likelihood.

## Minimal example

```python
import numpy as np
from bayesgbm import Config, GaussianPrior, Priors, StateModel, individual_fit
...
trials. BayesGBM never requires field names such as `reward` or `stimulus`.
...
BayesGBM deliberately separates optimizer curvature from Laplace curvature:

```text
multi-start L-BFGS-B
        -> final MAP
        -> independently recomputed central-FD observed posterior Hessian
...
BayesGBM also provides prior sensitivity and prior/posterior predictive checks.

## BMS and HBI

```python
from bayesgbm import bms, hbi_main
```
...
posterior moment matching and uses BayesGBM's MAP plus independently recomputed
...
Run the scientific reference scripts in `bayesgbm/dev/` after tests. They are
...
For the first BayesGBM archival release, update `CITATION.cff`, create a tagged
