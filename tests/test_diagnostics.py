...
from bayesgbm import convergence_diagnostics, posterior_hessian_diagnostics, prior_preconditioned_information, numerical_local_identifiability
...
def test_local_identifiability_detects_redundant_parameters():
    # p = sigmoid(phi0 + phi1): only the sum is observable.
    sig=lambda z:1/(1+np.exp(-z))
    model=StateModel(
        lambda x,th,u,y:x,
        lambda x,ph,u:sig(ph[0]+ph[1]),
        "bernoulli",
