"""Diagnostic plotting for BayesGBM individual fits."""
...
def _mpl():
    try:
        ...
        raise ImportError("plotting requires matplotlib; install BayesGBM[display]") from exc
    return plt
...
def _parameter_estimates_panel(ax, result, subject):
    ...
    ax.set_title("D  static parameters")
...
def _latent_panel(ax, result, subject):
    ...
    if est.get("uncertainty_type") == "propagated":
        ...
        title += " · filtered Gaussian shadow"
    ax.set_title(title)
