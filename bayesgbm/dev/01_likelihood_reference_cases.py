...
bern = StateModel(lambda x,t,u,y:x, lambda x,p,u:.8, "bernoulli", fixed, [0])
...
cat = StateModel(lambda x,t,u,y:x, lambda x,p,u:np.array([.2,.3,.5]), "categorical", fixed, [0])
out = cat.evaluate([0,0], {"y":np.array([2,1]), "u":None})["loglik"]
ref = np.log([.5,.3])
print("Categorical max error:", np.max(np.abs(out-ref)))
assert np.allclose(out, ref, atol=1e-12)

gau = StateModel(lambda x,t,u,y:x, lambda x,p,u:np.array([0.]), "gaussian", fixed, [0], observation_covariance=4.)
